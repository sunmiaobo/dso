...
bool CoarseInitializer::trackFrame(FrameHessian* newFrameHessian, std::vector<IOWrap::Output3DWrapper*> &wraps)
{
	...
	for(int lvl=pyrLevelsUsed-1; lvl>=0; lvl--)
	{
		...
		resetPoints(lvl);
		std::cout<< "refToNew_current.translation"<<refToNew_current.translation()<<std::endl;
		std::cout<< "refToNew_current.rotation"<<refToNew_current.rotationMatrix()<<std::endl;
		...
		bool plot)
{
	int wl = w[lvl], hl = h[lvl];
	Eigen::Vector3f* colorRef = firstFrame->dIp[lvl];
	//std::cout<< "firstFrame->dIp[lvl]"<<firstFrame->dIp[lvl]<<std::endl;
	//std::cout<< "colorRef"<<colorRef[0]<<colorRef[1]<<colorRef[2]<<std::endl;
	Eigen::Vector3f* colorNew = newFrame->dIp[lvl];
	//std::cout<< "colorNew"<<colorNew[0]<<colorNew[1]<<colorNew[2]<<std::endl;
	Mat33f RKi = (refToNew.rotationMatrix() * Ki[lvl]).cast<float>();
	//std::cout<< "Ki[lvl]"<<Ki[lvl]<<std::endl;
	Vec3f t = refToNew.translation().cast<float>();
	Eigen::Vector2f r2new_aff = Eigen::Vector2f(exp(refToNew_aff.a), refToNew_aff.b);
       // std::cout<< "r2new_aff[0]"<<r2new_aff[0]<<std::endl;
	//std::cout<< "r2new_aff[1]"<<r2new_aff[1]<<std::endl;
	float fxl = fx[lvl];
	float fyl = fy[lvl];
	float cxl = cx[lvl];
	float cyl = cy[lvl];
...
void CoarseInitializer::setFirst(	CalibHessian* HCalib, FrameHessian* newFrameHessian)
{
	...
	for(int lvl=0; lvl<pyrLevelsUsed; lvl++)
	{
		...
		int npts;
		if(lvl == 0)
			npts = sel.makeMaps(firstFrame, statusMap,densities[lvl]*w[0]*h[0],1,false,2);//在图像的每一层选点
		else
