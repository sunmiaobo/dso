...
void FullSystem::printResult(std::string file)
{
	...
	for(FrameShell* s : allFrameHistory)
	{
		...
		//if(s->marginalizedAt == s->id) continue;//setting_onlyLogKFPoses && 
...
void FullSystem::addActiveFrame( ImageAndExposure* image, int id )
{
	...
	FrameShell* shell = new FrameShell();//保存位置姿态信息
	...
	fh->shell = shell;
	allFrameHistory.push_back(shell);//保存所有轨迹


	// =========================== make Images / derivatives etc. =========================
	fh->ab_exposure = image->exposure_time;
    fh->makeImages(image->image, &Hcalib);//对图像进行预处理,对当前帧建立金字塔,并且计算每一层图像的梯度.
	...
	if(!initialized)
	{
		// use initializer!// 判断是否初始化,如果没有就将当前帧fh输入setfirst
		if(coarseInitializer->frameID<0)	// first frame set. fh is kept by coarseInitializer.
		{

			coarseInitializer->setFirst(&Hcalib, fh);//在图像的每一层选点
		}
